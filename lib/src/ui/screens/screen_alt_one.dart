...
class AltOne extends StatelessWidget {
  const AltOne({Key? key}) : super(key: key);
                    ...
                    state.formElements.isNotEmpty
                        ?  ListView.builder(
                              ...
                              itemBuilder: (BuildContext context, int idx) {
                                ...
                                final bool isTextField = element.type != FieldType.widget &&
                                    element.type != FieldType.dropdown &&
                                    element.type != FieldType.spacer &&
                                    element.type != FieldType.checkboxGroup &&
                                    element.type != FieldType.textLabel;

                                debugPrint('elem name ${element.name} type ${element.type} state ${element.state} ');
                                return isTextField
                                    ? SetFormField(formCubit: testForm, element: element)
                                    : Text(' coming soon  ${element.name} type ${element.type} state ${element.state} ');
                              })
                        : const Text('Form Loading '),
                    if (state.formElements.isNotEmpty)
                      SubmitButton(
                        label: 'submit',
                        callback: state.allVerified
                            ? () => testForm.resetBloc(
                                resetElements: form_bloc_utils.defaultForm(addDefault: true, firstName: 'Bruce', lastName: 'Wayne'))
                            : null,
                      ),
                  ],
                ),
...
class AppIconButton extends StatelessWidget {
  ...
  const AppIconButton({required this.icon, this.function, Key? key}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    return GestureDetector(
      onTap: function,
      child: Icon(icon, size: 25.0, color: function == null ? Colors.grey : Colors.blue),
    );
  }
}
